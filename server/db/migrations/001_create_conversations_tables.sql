-- Migration: Create conversations table for amora_db
-- This migration creates the conversations table with proper foreign key constraints
-- and ensures consistency with the existing users table structure
...
CREATE TABLE IF NOT EXISTS conversations (
    ...
    user_id VARCHAR(36) NOT NULL,  -- Changed from VARCHAR(255) to VARCHAR(36) to match users.id
    companion_id INT NOT NULL,
    title VARCHAR(255),
    last_message_id INT NULL,      -- Reference to the last message for quick access
    last_message_at TIMESTAMP NULL, -- When the last message was sent
    is_active BOOLEAN DEFAULT TRUE, -- Whether the conversation is still active
    ...
    CONSTRAINT fk_conversations_companion 
        FOREIGN KEY (companion_id) REFERENCES companions(id) 
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_conversations_last_message 
...
CREATE TABLE IF NOT EXISTS messages (
    ...
    sender_id VARCHAR(36) NOT NULL,  -- Changed from VARCHAR(255) to VARCHAR(36)
    content TEXT NOT NULL,
    message_type ENUM('text', 'image', 'audio', 'video', 'system') DEFAULT 'text',
    metadata JSON NULL,              -- Additional message metadata (e.g., image dimensions, audio duration)
    is_read BOOLEAN DEFAULT FALSE,
    read_at TIMESTAMP NULL,         -- When the message was read
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS notifications (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id VARCHAR(36) NOT NULL,  -- Changed from VARCHAR(255) to VARCHAR(36)
...
-- This ensures that when we create conversations, participants are automatically added
DELIMITER //
...
-- Create trigger to update conversation's last_message_id and last_message_at
DELIMITER //
...
-- Create trigger to update conversation's updated_at when messages are updated
DELIMITER //
